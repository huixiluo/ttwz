#!/usr/bin/env python3
"""v20: 先删除旧草稿 → 创建新文章 → 输入内容 → 保存

核心思路：
1. 通过API删除旧草稿(pgc_id=7673773338287751730)
2. 打开发布页面，此时没有旧草稿，应该创建新文章
3. 输入内容并保存
"""
...
async def delete_old_drafts(context):
    """通过API删除旧草稿"""
    print("  [0] 检查并删除旧草稿...")
    ...
    async def on_response(response):
        if 'draft_list' in response.url or 'article/delete' in response.url:
            try:
                body = await response.text()
                save_bodies.append(f"{response.status} {response.url[:120]}\n  {body[:500]}")
            except: pass
    ...
    draft_data = await page.evaluate("""
        async () => {
            try {
                const resp = await fetch('/mp/agw/creator_center/draft_list?type=2&count=20&app_id=1231');
    ...
    if draft_data and draft_data.get('draft_list'):
        drafts = draft_data['draft_list']
        print(f"    找到 {len(drafts)} 篇草稿")
        for d in drafts:
            pgc_id = d.get('pgc_id', '')
            title = d.get('title', '')[:40]
            print(f"      pgc_id={pgc_id} | {title}")
        ...
        for d in drafts:
            ...
            if pgc_id:
                result = await page.evaluate(f"""
                    async () => {{
                        try {{
                            const resp = await fetch('/mp/agw/creator_center/draft/delete', {{
                                method: 'POST',
                                headers: {{'Content-Type': 'application/json'}},
                                body: JSON.stringify({{pgc_id: '{pgc_id}', app_id: '1231'}})
                            }});
                            const data = await resp.json();
                            return data;
                        }} catch(e) {{ return null; }}
                ...
                """)
                status = result.get('code') if result else 'error'
                print(f"      删除 pgc_id={pgc_id}: code={status}")
                await asyncio.sleep(1)
    else:
        print("    草稿箱为空，无需删除")
...
async def process_article(context, art, index, total):
    ...
    try:
        ...
        """)
        print(f"  编辑器初始内容: {init_content[:100]}")
        ...
        await page.screenshot(path=f"/workspace/v20_art{index}.png")
        ...
        traceback.print_exc()
        try:
            await page.screenshot(path=f"/workspace/v20_art{index}_err.png")
        except: pass
        try:
            await page.close()
        except: pass
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # 先删除旧草稿
        await delete_old_drafts(context)
        ...
        await verify_page.screenshot(path="/workspace/v20_draft_final.png")
