...
def main():
    ...
    co.set_address("127.0.0.1:9235")
    co.set_user_data_path(os.path.join(BASE_DIR, ".chrome_profile_edit4"))
...
""")
    ...
    # 获取草稿标题列表
    titles_info = page.run_js(f"""
var ourTitles = {json.dumps(our_titles)};
var titleLinks = document.querySelectorAll('a.title');
var result = [];
for (var i = 0; i < titleLinks.length; i++) {{
    var titleText = titleLinks[i].textContent.trim();
    var isOurs = false;
    for (var t of ourTitles) {{
        if (titleText.indexOf(t) >= 0) {{ isOurs = true; break; }}
    }}
    result.push({{index: i, title: titleText, isOurs: isOurs}});
}}
return JSON.stringify(result);
""")

    try:
        titles = json.loads(titles_info) if titles_info else []
    except:
        titles = []

    print(f"  找到 {len(titles)} 个草稿")
    ...
    for t in titles:
        if not t["isOurs"]:
            target_idx = t["index"]
            print(f"  选择: [{target_idx}] {t['title'][:40]}")
            break
    ...
    # 用DrissionPage的元素点击方法
    print(f"\n  点击草稿标题...")
    try:
        title_el = page.ele(f'css:a.title:nth-of-type({target_idx + 1})', timeout=5)
        if title_el:
            title_el.click()
            print("  点击成功")
        else:
            ...
            page.run_js(f"""
var titleLinks = document.querySelectorAll('a.title');
if ({target_idx} < titleLinks.length) {{
    titleLinks[{target_idx}].click();
}}
""")
    except Exception as e:
        print(f"  DrissionPage点击失败: {e}，用JS点击")
        page.run_js(f"""
var titleLinks = document.querySelectorAll('a.title');
if ({target_idx} < titleLinks.length) {{
    titleLinks[{target_idx}].click();
...
""")
    ...
    try:
        ...
        if len(tab_ids) > 1:
            print(f"  发现{len(tab_ids)}个标签页，切换到新标签页")
            page = page.get_tab(tab_ids[-1])
            time.sleep(3)
