...
def fill_title(page, title):
    ...
    if title_el:
        ...
        time.sleep(0.5)
        # 触发blur事件，确保React的onChange被正确触发
        page.run_js("""
...
def save_draft(page, timeout=60):
    """主动点击保存草稿按钮，等待保存成功确认"""
    print("  保存草稿...")
    
    # 先等待一下，让React处理完title和content的变更
    time.sleep(3)
...
return 'not_found';
""")
        print(f"  查找保存按钮(尝试{attempt+1}): {save_clicked}")
