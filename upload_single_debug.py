...
async def save_requests(response):
    """收集保存相关请求"""
    url = response.url
    if any(kw in url for kw in ['save', 'draft', 'auto_save', 'article']):
        try:
            body = await response.text()
            print(f"  [NET] {response.status} {response.request.method} {url[:120]}")
            if len(body) < 2000:
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        page.on('response', save_requests)
  ...
  return JSON.stringify({status:'error',error:e.message});
        ...
        try:
            ...
            if pm_data.get('status') == 'ok':
                print(f"  [OK] PM设置成功: {pm_data.get('chars')}字, {pm_data.get('imgs')}张图")
        except: pass
        ...
        await page.screenshot(path=f"/workspace/debug_art{article_index}.png")
        await browser.close()
        print(f"完成")
