#!/usr/bin/env python3
"""v18: 键盘输入 + 图片粘贴方案，完全不用PM API

策略：
1. 逐张上传图片获取服务器URL
2. 打开全新发布页面
3. 按布局顺序：键盘输入文字段落 → 粘贴图片 → 继续文字 → ...
4. 等待自动保存
"""
...
async def upload_images_get_urls(page, img_bytes_list):
    ...
    for idx, img_bytes in enumerate(img_bytes_list):
        print(f"    图{idx+1}/{len(img_bytes_list)}: ", end="", flush=True)
    ...
    async def capture_response(response):
        ...
        if any(kw in url for kw in ['save', 'draft', 'auto_save', 'article/new', 'article/edit', 'article/publish', 'article/update']):
            try:
                ...
                short = body[:500] if len(body) < 2000 else body[:500] + '...'
                save_responses.append(f"{response.status} {response.request.method} {url[:120]}\n  {short}")
            except: pass

    # 创建上传图片用的临时页面
    upload_page = await context.new_page()
    upload_page.on('response', capture_response)

    try:
        # 阶段1: 打开临时页面，上传所有图片获取URL
        ts = int(time.time() * 1000)
        print(f"  [1] 上传图片阶段...")
        await upload_page.goto(f"{PUBLISH_URL}?_t={ts}", wait_until="domcontentloaded", timeout=30000)
        await asyncio.sleep(5)
        await dismiss_notifications(upload_page)

        for i in range(20):
            await asyncio.sleep(1)
            ready = await upload_page.evaluate("""
                () => { const ed = document.querySelector('.ProseMirror'); return ed && ed.getBoundingClientRect().width > 0; }
            """)
            if ready: break
        else:
            print("  [ERROR] 上传页面编辑器未就绪")
            return False

        image_urls = await upload_images_get_urls(upload_page, img_bytes_list)
        valid_urls = [u for u in image_urls if u]
        print(f"  上传结果: {len(valid_urls)}/{len(img_bytes_list)}张成功")
        await upload_page.close()

        # 阶段2: 打开全新发布页面，按布局输入内容
        ts2 = int(time.time() * 1000)
        print(f"  [2] 内容输入阶段...")
        page = await context.new_page()
        page.on('response', capture_response)
        await page.goto(f"{PUBLISH_URL}?_t={ts2}", wait_until="domcontentloaded", timeout=30000)
        await asyncio.sleep(5)
        ...
        await page.screenshot(path=f"/workspace/v18_art{index}.png")
        # 打印网络请求
        if save_responses:
            print(f"  [NET] 保存相关请求:")
        ...
        traceback.print_exc()
        try:
            await page.screenshot(path=f"/workspace/v18_art{index}_err.png")
        except: pass
        return False
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for i, art in enumerate(articles, 1):
            try:
                # 每篇文章使用独立browser context，避免加载旧草稿
        ...
        await verify_page.screenshot(path="/workspace/v18_draft_final.png")
