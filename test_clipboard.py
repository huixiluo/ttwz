...
# 先尝试纯文本（不含base64图片），看是否保存成功
# 用[图片]占位符替换base64图片
body_text_only = re.sub(r'<img[^>]*src="data:image/[^"]*;base64,[^"]*"[^>]*>', '[图片]', body_html)
body_text_only = re.sub(r'<[^>]+>', '', body_text_only)

print(f"正文长度: {len(body_text_only)} 字")
...
if editor:
    ...
    time.sleep(1)
    # Ctrl+A 全选，Ctrl+V 粘贴
    page.actions.key_down('CTRL').key_down('A').key_up('A').key_up('CTRL').perform()
    time.sleep(0.3)
    page.actions.key_down('CTRL').key_down('V').key_up('V').key_up('CTRL').perform()
    time.sleep(2)
    print("  已粘贴")

# 在编辑器输入一个空格再删除，触发React
editor = page.ele('.ProseMirror', timeout=5)
if editor:
    editor.click()
    time.sleep(0.5)
