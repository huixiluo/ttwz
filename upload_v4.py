...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for i, art in enumerate(articles, 1):
            ...
            await page.keyboard.type(" ", delay=50); await asyncio.sleep(0.3)
            await page.keyboard.press("Backspace"); await asyncio.sleep(0.5)
                ...
                for _ in range(60):
                    ...
                    urls = await page.evaluate("""
                        () => {
                            ...
                            const all = Array.from(imgs).map(i => i.src);
                            return all.every(s => s && !s.startsWith('blob:') && !s.startsWith('data:'));
                        }
                    """)
                    if urls: break
            ...
            image_urls = await page.evaluate("""
                () => {
                    const imgs = document.querySelectorAll('.ProseMirror img');
                    return Array.from(imgs).map(i => i.src).filter(s => s && !s.startsWith('blob:') && !s.startsWith('data:'));
                }
            """)
