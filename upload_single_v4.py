#!/usr/bin/env python3
"""单篇上传v4 - 拦截article/edit请求，阻止加载旧草稿，使用新pgc_id"""
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # 拦截article/edit请求，阻止加载旧草稿
        async def block_edit_route(route):
            if 'article/edit' in route.request.url:
                print("  拦截article/edit请求，返回空响应")
                await route.fulfill(status=200, body='{"code":0,"data":{},"err_no":0,"message":"success"}')
            else:
...
try{
  ...
  return JSON.stringify({status:'error',error:e.message});
        ...
        await page.screenshot(path=f"/workspace/v4_art{article_index}.png")
        await browser.close()
        print(f"完成")
