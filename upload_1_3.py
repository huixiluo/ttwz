...
async def upload_article(page, art):
    title = art["title"]
    ...
    try:
        ...
        print("  [ERROR] 编辑器未就绪"); return False

    # 初始化编辑器：点击并输入一些文字，触发pgc_id初始化
    editor = page.locator(".ProseMirror").first
    await editor.click(); await asyncio.sleep(0.5)
    await page.keyboard.type("初始化内容...", delay=50); await asyncio.sleep(2)
...
async def main():
    ...
    # 只处理第1篇和第3篇
    targets = [articles[0], articles[2]]
    print(f"上传 {len(targets)} 篇文章\n{'='*60}")
    ...
    async with async_playwright() as p:
        ...
        for i, art in enumerate(targets, 1):
            try:
                if await upload_article(page, art):
                    success += 1
    ...
    print(f"\n{'='*60}")
    print(f"完成: {success}/{len(targets)} 篇")
