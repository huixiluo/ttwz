...
4. 文章整体导向积极正能量，站在读者角度，引发共鸣；结尾靠内容本身留余味，不靠模板化的互动引导语硬拉评论；
...
8. 【结尾——非常重要，必须严格执行】结尾必须多样、克制，坚决杜绝模板化收尾：
   - 严禁以下固化结尾模式（违反即视为不合格）：
     * "提问+评论区"模板：如"你怎么看？评论区聊聊""评论区说说你的看法""你怎么认为？评论区聊"——提问后必须紧跟"评论区"引导语的模式一律禁止；
     * 模板化互动引导语：如"欢迎留言讨论""大家怎么看呢""说说你的观点"等万能尾巴；
     * 总结升华式：如"这不仅xxx，更是xxx的体现""时间会给出答案"这类空泛收束；
   - 【结尾手法硬性要求】每篇文章的结尾必须从以下手法中选择最贴合的一种，且同一批次文章中相邻两篇不得使用相同手法：
     * 画面留白式：收在一个具体画面、场景或动作上，戛然而止，让余味留在画面里（如"视频里的笑容还没散，人已经走了"）；
     * 观点直陈式：一句话亮明态度，干脆利落，不解释不铺垫（如"在巅峰期主动换赛道，总比熬到走投无路再转身体面"）；
     * 数字回扣式：用文中出现过的某个关键数字收尾，让数字自己说话（如"平均每天近300万个包裹从这里飞往全球200多个国家"）；
     * 时间展望式：往后看一步，留一点悬念或期待，不下定论（如"10月的U17世界杯，才是这批小将真正的大考"）；
     * 金句式：一句凝练有力的话收束全文，可带一点情绪或态度（如"潮流绕了快三十年，自己转回来找她"）；
     * 细节呼应式：回扣开头提到的某个细节、人物或画面，首尾形成闭环；
     * 开放提问式：抛一个具体、克制的问题收尾，问题必须紧扣文中具体事实，不得泛泛而问，且提问后不得附加任何"评论区"引导语；
   - 结尾不得出现任何总结性、过渡性词汇（同上文风格要求）；
   - 提问式结尾在同一批次中最多出现1篇，优先考虑其余6种手法。
...
5. 【结尾去模板化】若原文结尾是"提问+评论区聊聊"式模板（如"你怎么看？评论区聊聊""评论区说说看法"）或其他万能互动引导语（"欢迎留言讨论""大家怎么看"），必须改写为以下手法之一：画面留白式（收在具体画面上）/ 观点直陈式（一句话亮态度）/ 数字回扣式（用文中关键数字收）/ 时间展望式（往后看一步）/ 金句式（一句凝练的话收）/ 细节呼应式（回扣开头细节）。同一批次相邻文章结尾手法不得重复，提问式结尾最多保留1篇且不得带"评论区"引导语；
6. 允许有轻微不完美，还原普通人真实输出的语感，不要写得像范文或满分作文；
7. 禁止堆砌网络热梗、禁止强行口语化、禁止编造故事或细节；
8. 不要套用别的模板风格，保持原文的整体基调和段落结构，只做行文层面的打磨；
9. 【字数硬性要求】改写后正文字数必须高于600字（>600）。可适度精简，但不得低于600字；若删减后会低于600字，请补充适当内容维持字数。
