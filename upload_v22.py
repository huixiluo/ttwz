#!/usr/bin/env python3
"""v22: 最小化版本 - 只拦截article/edit返回404，不删除旧草稿"""
...
async def run():
    ...
    async with async_playwright() as p:
        ...
        for art_idx, art in enumerate(articles):
                    ...
                    try:
                        ...
                        if 'article/new' in url:
                            try:
                                ...
                                if pid and pid != '0':
                                    new_pgc_id[0] = pid
                                    print(f"  [NEW] pgc_id={pid}", flush=True)
                            except: pass
            ...
            async def block_edit_route(route):
                if 'article/edit' in route.request.url:
                    print(f"  [BLOCK] article/edit → 404", flush=True)
            ...
            try:
                ...
                for _ in range(10):
                    ...
                    await asyncio.sleep(1)
                print(f"  pgc_id: {new_pgc_id[0] or '未获取'}", flush=True)
                    ...
                    if para_num in image_layout:
                        ...
                        for _ in range(num_imgs):
                            if img_idx < len(img_bytes_list):
                                b64 = base64.b64encode(img_bytes_list[img_idx]).decode('ascii')
                                await page.evaluate("""
                                    (b64) => {
                ...
                await page.screenshot(path=f"/workspace/v22_art{i}.png")
                ...
                traceback.print_exc()
                try:
                    await page.screenshot(path=f"/workspace/v22_art{i}_err.png")
                except: pass
                try:
                    await page.close()
                except: pass
        ...
        await verify_page.screenshot(path="/workspace/v22_draft_final.png")
