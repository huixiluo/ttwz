...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # 找到吴越的草稿并删除
        if "吴越深夜辟谣" in draft_text:
            print("  发现旧草稿(吴越)，正在删除...")
            try:
                # 点击第一个草稿的删除按钮
                delete_btn = page.locator("text=删除").first
...
try{
  ...
  return JSON.stringify({status:'error',error:e.message});
        ...
        await page.screenshot(path=f"/workspace/v3_art{article_index}.png")
        await browser.close()
        print(f"完成")
