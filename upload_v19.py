#!/usr/bin/env python3
"""v19: 修复版 - 拦截article/edit + 捕获新pgc_id + 单阶段流程

核心修复：
1. route拦截 article/edit 请求，返回空响应防止旧内容加载
2. 捕获 article/new 响应中的 pgc_id
3. 单阶段流程：上传图片→清空编辑器→按布局输入→保存
4. 更可靠的封面上传
"""
...
async def clear_editor(page):
    ...
    await page.evaluate("""
        () => {
            ...
            if (ed && ed.pmViewDesc && ed.pmViewDesc.node) {
                const view = ed.pmViewDesc.node;
                return;
            }
            // Fallback: 全选删除
            if (ed) {
...
async def upload_cover(page, cover_paths):
    ...
    for ci, cf in enumerate(valid):
        ...
        try:
            ...
            try:
                await page.evaluate("""
                    () => { const add = document.querySelector('.article-cover-add'); if (add) { add.click(); } }
                """)
                await asyncio.sleep(1)
            except: pass
...
async def process_article(context, art, index, total):
    ...
    async def capture_response(response):
        url = response.url
        if any(kw in url for kw in ['save', 'draft', 'auto_save', 'article/new', 'article/edit', 'article/publish', 'article/update']):
            try:
                body = await response.text()
                short = body[:800] if len(body) < 2000 else body[:500] + '...'
                save_responses.append(f"{response.status} {response.request.method} {url[:150]}\n  {short}")
                # 检查是否保存成功
                if 'article/publish' in url or 'auto_save' in url:
                    if '"code":0' in body or '"err_no":0' in body:
                        save_success[0] = True
                # 提取新pgc_id
                if 'article/new' in url:
                    try:
                        data = json.loads(body)
                        pid = data.get('data', {}).get('pgc_id') or data.get('data', {}).get('article_pgc', {}).get('pgc_id')
                        if pid and pid != '0':
                            new_pgc_id[0] = pid
    ...
    # 拦截 article/edit 请求，防止加载旧内容
    async def block_edit_route(route):
        if 'article/edit' in route.request.url:
            print(f"  [BLOCK] 拦截 article/edit 请求")
            await route.fulfill(status=200, body='{"code":0,"data":{"article_pgc":{"pgc_id":"0","content":"","title":"","abstract":""}},"err_no":0,"message":"success"}')
        else:
    ...
    try:
        ...
        # 等待article/new返回pgc_id
        for i in range(10):
            if new_pgc_id[0]: break
            await asyncio.sleep(1)
        if not new_pgc_id[0]:
            print("  [WARN] 未获取到新pgc_id，继续尝试...")
        else:
            print(f"  [OK] 新pgc_id={new_pgc_id[0]}")

        # 清空编辑器（旧内容可能已通过article/edit加载）
        await clear_editor(page)
        await asyncio.sleep(0.5)

        # 确保编辑器为空
        empty_check = await page.evaluate("""
            () => { const ed = document.querySelector('.ProseMirror'); return ed ? ed.innerText.length : -1; }
        """)
        print(f"  编辑器内容长度: {empty_check}")
        ...
        await page.screenshot(path=f"/workspace/v19_art{index}.png")
        ...
        traceback.print_exc()
        try:
            await page.screenshot(path=f"/workspace/v19_art{index}_err.png")
        except: pass
        try:
            await page.close()
        except: pass
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        await verify_page.screenshot(path="/workspace/v19_draft_final.png")
