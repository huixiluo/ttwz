#!/usr/bin/env python3
"""通过键盘逐段输入方式设置内容，确保ProseMirror检测到变更"""
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        async def on_response(response):
            if 'publish' in response.url and response.request.method == 'POST':
                try:
                    ...
                    # Only log if content is not empty
                    if '"content_word_cnt":0' not in body:
                        print(f'  [SAVE OK] {body[:200]}')
                    else:
                        print(f'  [SAVE EMPTY] {body[:150]}')
                except:
                    pass
        ...
        for pi, para_text in enumerate(paragraphs):
            ...
            await editor.click()
            await asyncio.sleep(0.3)
            await page.keyboard.type(para_text, delay=10)
            await asyncio.sleep(0.5)
