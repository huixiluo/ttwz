...
""") or ""
        ...
        if responses == 'SUCCESS' or status == 'SAVED':
            print(f"  [save] 保存成功! (用时{i*2}s, status={status})")
            dlog(f"wait_for_save: 成功 (用时{i*2}s)")
            return True

        if status != last_status:
            print(f"  [save] [{i*2}s] status={status}")
            dlog(f"wait_for_save: [{i*2}s] status={status}")
            last_status = status

        if i % 10 == 0 and i > 0:
            print(f"  [save] [{i*2}s] status={status}")

    print(f"  [save] 保存超时 (最后status={status})")
    dlog(f"wait_for_save: 超时 (status={status})")
    ...
    try:
        ...
        for r in rs[-3:]:
            body = r.get('body','')[:300]
            print(f"  [save] API响应: {body}")
            dlog(f"wait_for_save: API响应: {body}")
    except:
        pass
    return False
...
def main():
    ...
    co.set_address("127.0.0.1:9234")
    co.set_user_data_path(os.path.join(BASE_DIR, ".chrome_profile_edit3"))
...
for (var a of allLinks) {{
    if (a.textContent.trim() === '编辑') {{
        ...
        editBtns.push({{title: titleText, isOurs: isOurs}});
    }}
}}
...
""")
    ...
    # 选择第一个不是我们的草稿
    target_idx = None
    for i, eb in enumerate(edit_btns):
        if not eb["isOurs"]:
            target_idx = i
            print(f"  选择: [{i}] {eb['title'][:40]}")
            break
...
return '{{}}';
""")
    print(f"  编辑按钮属性: {edit_href[:300]}")

    # 尝试方法1: 直接获取编辑链接的href并导航
    try:
        edit_data = json.loads(edit_href)
        href = edit_data.get("href", "")
        if href and href != "":
            print(f"  导航到: {href}")
            page.get(href)
            time.sleep(5)
...
""")
    ...
    if len(tabs) > 1:
        ...
        for tab_id in tabs[1:]:
            page = page.get_tab(tab_id)
            break
        time.sleep(3)
        print(f"  新标签页URL: {page.url}")
