# -*- coding: utf-8 -*-
"""通过ProseMirror API直接设置内容并触发React状态更新"""
...
# 第2步：查找React fiber并触发状态更新
page.run_js("""
// 查找React内部状态
var editor = document.querySelector('.ProseMirror');
if (editor) {
    // 获取React fiber
    var key = Object.keys(editor).find(function(k) { return k.startsWith('__reactFiber') || k.startsWith('__reactInternalInstance'); });
    if (key) {
    ...
    // 对于contentEditable，触发selectionchange
    document.dispatchEvent(new Event('selectionchange', {bubbles: true}));
}
""")
time.sleep(1)

# 第3步：使用DrissionPage在编辑器里输入几个字符再删除
