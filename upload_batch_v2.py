#!/usr/bin/env python3
"""批量上传文章到头条草稿箱 v2 - 浏览器自动化 + 粘贴HTML内容"""
...
async def paste_html_content(page, content_html):
    """通过粘贴HTML内容到编辑器"""
    ...
    await page.evaluate(f"""
        () => {{
            ...
            const cd = {{
                types: ['text/html'],
                getData: function(type) {{ return type === 'text/html' ? html : ''; }},
                setData: function() {{}},
                clearData: function() {{}},
                files: [],
                items: []
...
async def process_article(page, art, index, total):
    ...
    # 粘贴内容到编辑器
    print(f"  粘贴内容到编辑器...")
    await paste_html_content(page, content_html)
    ...
    result = await page.evaluate("""
        () => {
            ...
            const lines = all.split('\\n').filter(l => l.includes('保存') || l.includes('成功') || l.includes('失败'));
