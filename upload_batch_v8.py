...
async def process_article(page, art, index, total):
    ...
    # 截图确认
    await page.screenshot(path=f"/workspace/editor_before_preview_{index}.png")
    ...
    try:
        ...
        if await preview_btn.is_visible(timeout=5000):
            ...
            # 截图预览页
            await page.screenshot(path=f"/workspace/preview_page_{index}.png")
            
            # 如果在预览页，返回发布页
            if "preview" in current_url.lower():
                print(f"    在预览页，返回...")
                await page.goto("https://mp.toutiao.com/profile_v4/graphic/publish", wait_until="domcontentloaded", timeout=15000)
                await asyncio.sleep(3)
            return True
        else:
            print(f"    预览按钮不可见")
