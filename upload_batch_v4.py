#!/usr/bin/env python3
"""批量上传文章到头条草稿箱 v4 - 逐段键盘输入 + 预览并发布保存"""
...
async def process_article(page, art, index, total):
    ...
    try:
        ...
        if await publish_btn.is_visible(timeout=5000):
            ...
            # 尝试关闭预览弹窗
            try:
                close_btn = page.locator("button:has-text('关闭'), button:has-text('返回'), .close-btn").first
            ...
            # 如果页面跳转了，需要回到编辑器
            if "publish" not in current_url and "preview" in current_url.lower():
